  ...
  return (
    <>
      <h4 className="display-4">
        {!film ? "Part 3 Incomplete" : film.title ? film.title : ""}
      </h4>
      <br />
      <Row as="dl">
        {!film ? (
          C.INCOMPLETE_3_FULL
        ) : filmElements.length > 0 ? (
          filmElements
        ) : (
          <></>
        )}
  ...
  return Object.keys(film).map((key) => {
    if (key in C.KEY_LABELS) {
      let formattedValue = !filmChildren[key]
        ...
        ? filmChildren[key].map((item, index) => (
            <React.Fragment key={`${filmChildren[key]}-${index}`}>
              {item.name}
              <br />
            </React.Fragment>
          ))
...
const generateChildren = (film, updateFunction) => {
  ...
  Object.entries(film).map(async ([key, value]) => {
    ...
    if (value instanceof Array) {
      if (
        value.length !== 1 ||
        value[0].lastIndexOf("/") !== value[0].length - 1
      ) {
    ...
    updateFunction((prevFilmChildren) => {
      return { ...prevFilmChildren, [key]: formattedValue };
    });
  });
};
